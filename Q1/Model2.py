...
class CovidCasesForecast:
    def __init__(self, file_path, variable = "total_cases", iso_code = 'FRA', time_step = 7):
...
if __name__ == '__main__':
    forecast = CovidCasesForecast(file_path='Assets/owid-covid-data.csv', iso_code='FRA', variable='total_cases', time_step=7)
    forecast.fit_transform()
    forecast.save_model('model2_totalCases.h5')
    forecast.evaluate_model()
    forecast.predict_train_val()
    forecast.forecast(74, date(2023,1,16))
    forecast.save_forecast('tcases_forecasted.csv')
    print("DONE!!!")
